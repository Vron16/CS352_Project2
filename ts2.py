...
def handle_connection(connection_socket, ts_dns_table):
    ...
    print("[TS2]: Received request to find IP address for the hostname:", client_query)
    ...
    if client_query in ts_dns_table:
        ...
        print("[TS2]: Query Found.Sending back to LS:", address_response)
        connection_socket.send(address_response.encode('utf-8'))
    else:
        error_response = client_query + " - Error:HOST NOT FOUND"
        print
        "[TS2]: Could not locate query in top-level server DNS table. Error:HOST NOT FOUND."
...
def ts_server():
    if len(sys.argv) != 2:
        raise TypeError('[TS2]: Expected 2 but received %d arguments.' % len(sys.argv))
    ...
    while line:
        ...
        if flag == 'A':
            ...
            raise ValueError("[TS1]: Error - PROJ2-DNSTS2.txt file contains unrecognized flags.")
        line = input_file.readline()
    ...
    try:
        ...
        print("[TS2]: Could not create socket for root server to listen to connections from due to error:", err)

    # Bind the server socket to the IP address of the machine it is running on and the port provided as input.
    host = mysoc.gethostname()
    print("[TS2]: Top-level server host name is:", host)
    host_ip = mysoc.gethostbyname(host)
    print("[TS2]: Top-level server host IP address is:", host_ip)
    ...
    while 1:
        ...
        print("[TS2]: Received a connection request from a client at:", addr)
